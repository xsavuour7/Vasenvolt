...
class AnomalyItem(BaseModel):
    """Schema for a telemetry anomaly preview item."""

    timestamp: datetime = Field(..., description="Timestamp of the anomalous reading", example="2024-01-15T10:30:00Z")
    meter_id: int = Field(..., description="Meter ID", example=1)
    meter_name: Optional[str] = Field(None, description="Meter name", example="Main Meter")
    site_id: int = Field(..., description="Site ID", example=1)
    site_name: Optional[str] = Field(None, description="Site name", example="Main Campus")
    kwh: float = Field(..., description="Current kWh reading", example=150.0)
    previous_kwh: float = Field(..., description="Previous kWh reading", example=100.0)
    deviation_percent: float = Field(..., description="Absolute percentage deviation from previous point", example=50.0)